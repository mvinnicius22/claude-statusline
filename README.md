...
A multi-line status bar for [Claude Code](https://code.claude.com/docs/en/statusline): context window, subscription usage limits, token/cache economics, and git state — rendered in the terminal from the data Claude Code feeds the statusline.
...
| `⚡high` | [Reasoning effort](https://code.claude.com/docs/en/statusline), color-coded: `low` green · `medium` yellow · `high` magenta · `xhigh`/`max` red. |
| `default` | Current [output style](https://code.claude.com/docs/en/output-styles). `default` is the built-in style; change it with `/output-style`. |
...
Read from `rate_limits` in the [statusline input](https://code.claude.com/docs/en/statusline) — no API call, no token cost. These lines appear only on **Pro/Max** plans, where Claude Code provides this data; on API/enterprise billing they're omitted.
...
Cumulative token counts for the session, parsed from the transcript (cached by file mtime, so a large session isn't re-read every refresh). Each type is billed at a different multiple of the base input price, per the [prompt caching docs](https://platform.claude.com/docs/en/build-with-claude/prompt-caching):
...
- Subscription limits and cost come from the data Claude Code already pipes to the statusline ([statusline docs](https://code.claude.com/docs/en/statusline)) — no network calls, no API key, no extra tokens.
